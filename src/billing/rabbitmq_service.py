...
class RabbitMQService:
    ...
    def __init__(self):
        if not self._is_initialized:
            ...
            self.channel = None
            self.queue_name = "premiumUsersQueue"
    ...
    def connect(self):
        ...
        self.channel.queue_declare(
            queue=f"{self.queue_name}-canceled",
            durable=True,
            arguments=arguments
        )
    ...
    def publish_message(self, message) -> bool:
        ...
        try:
            ...
            # logger.info(f"Successfully published create WOD message for user: {message.get('email', 'unknown')}")
            return True
        except Exception as e:
            logger.error(f"Failed to publish message to RabbitMQ: {str(e)}", exc_info=True)
            return False
    def publish_message_cancel(self, message) -> bool:
        """Publish a message to the queue"""
        try:
            self.ensure_connection()
